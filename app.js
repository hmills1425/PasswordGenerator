...
// Function that returns a random lowecase letter using a random index in the "letters" string
...
// Example of the getRadomLower function
console.log(getRandomLower());// Randon Lowercase letter
...
function getRadomSymbol(){
    ...
    return symbols[randomIndex(sybols)];
};
...
const generateE1 = document.querySelector(`#generate`);

// Generate Password Function (Function that accepts true or false values as well as an argument)
// NOTE: The checkbox inputs and number (AKA length) will determine the value/arguments entered into this function
function generatePassword(lower, upper, number, symbol, length){
    ...
    // If the user has NOT selected any of the four options, then the alert will be displayed and an empty string will be returned from the function so the password displays to the user will be an emply string (AKA Nothing)
    if (typesCount === 0){
    ...
    // Creating an array of arrays. The first item in each nested array holds the value of a string that will be used to access a function in teh randomFunctions object. Also, the second item in each nested array are of the values passed into this generatePassword function
    let typesArr = [
        [`lower`, lower],
        [`upper`, upper],
        [`number`, number],
    ...
    // Checking if the value for the index of 1 in each item (AKA Array) in the typesArr array is true or false. Also, removing this item from the typesArr array if it is false.
    ...
    for (i = 0; i< length; i+= typesCount){
        // One of the items in the updated/filtered version of the typesArray will be value/argument passed in for the types parameter each time the anonymous arrow function is run
...
generateE1.addEventListener(`click`, () => {
    ...
    // The generatePassword function takes the true/false values determined by the checkboxes as well as the number from the ionput as arguments and returns a string (AKA The Password) which is set as the innerText value for the "result" 
    (AKA Span) Element
